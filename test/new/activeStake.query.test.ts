...
function loadQueryNode (name: string): Promise<DocumentNode> {
  return util.loadQueryNode(path.resolve(__dirname, '..', 'new', 'example_queries', 'active_stake'), name)
}
...
describe('activeStake', () => {
  ...
  it('can return active stake snapshots for an address', async () => {
    ...
    util.saveResult(result.data, "new",   'active_stake', "activeStakeForAddress.json");
  ...
  it('can return aggregated active stake information for an address', async () => {
    ...
    util.saveResult(result.data, "new",   'active_stake', "averageActiveStakeForAddres.json");

    // FIX_FAIL expect(activeStake_aggregate.aggregate.count).toBeDefined()
  })
})
