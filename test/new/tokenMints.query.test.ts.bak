...
function loadQueryNode (name: string): Promise<DocumentNode> {
  return util.loadQueryNode(path.resolve(__dirname, '..', 'new', 'example_queries', 'token_mints'), name)
}
...
describe('tokenMints', () => {
  ...
  it('can return information on token minting and burning', async () => {
    ...
    util.saveResult(result.data, "new",   "token_mints", "tokenMints_1.json");
  ...
  it('can return information on assets by fingerprint', async () => {
    ...
    util.saveResult(result.data, "new",   "token_mints", "tokenMints_2.json");
