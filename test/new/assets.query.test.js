...
function loadQueryNode (name) {
  return util.loadQueryNode(path.resolve(__dirname, '..', 'new', 'example_queries', 'assets'), name)
}
...
describe('assets', () => {
  ...
  it('can return information on assets', async () => {
    ...
    util.saveResult(result.data, "new",   "assets", "assets_1.json");

    const { assets_aggregate, assets } = result.data;
    const { aggregate } = assets_aggregate
    // FIX FAIL expect(aggregate.count).toBeDefined()
    // expect(assets.length).toBeGreaterThan(0)
    // expect(assets[0].tokenMints.length).toBeGreaterThan(0)
    // FIX FAIL expect(parseInt(assets[0].tokenMints_aggregate.aggregate.count)).toBeGreaterThan(0)
    // expect(assets[0].fingerprint.slice(0, 5)).toBe('asset')
  })

  // FIX FAIL it('can return information on assets by fingerprint', async () => {
  //   const result = await client.query({
  //     query: await loadQueryNode('assets'),
  //     variables: {
  //       where: {
  ...
  //   util.saveResult(result.data, "new",   "assets", "assets_2.json");
  ...
  // FIX FAIL it('can return information on assets by assetId', async () => {
  //   const result = await client.query({
  //     query: await loadQueryNode('assets'),
  //     variables: {
  //       where: {
  ...
  //   util.saveResult(result.data, "new",   "assets", "assets_3.json");
