...
function loadQueryNode (name: string): Promise<DocumentNode> {
  return util.loadQueryNode(path.resolve(__dirname, '..', 'new', 'example_queries', 'stake_pools'), name)
}
...
describe('stakePools', () => {
  ...
  it('can lookup stake pools by ID', async () => {
    ...
    util.saveResult(result.data, "new",   "stake_pools", "stakePoolById.json");
  ...
  it('can return details on active stake pools', async () => {
    ...
    util.saveResult(result.data, "new",   "stake_pools", "allStakePoolFields.json");
  ...
  it('can return aggregated data on all stake pools', async () => {
    ...
    util.saveResult(result.data, "new",   "stake_pools", "aggregateStakePoolSummary_1.json");
  ...
  it('can return aggregated data on active stake pools', async () => {
    ...
    util.saveResult(result.data, "new",   "stake_pools", "aggregateStakePoolSummary_2.json");
  ...
  it('can return aggregated data on retiring stake pools', async () => {
    ...
    util.saveResult(result.data, "new",   "stake_pools", "aggregateStakePoolSummary_3.json");
