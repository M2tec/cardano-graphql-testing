...
function loadQueryNode (name: string): Promise<DocumentNode> {
  return util.loadQueryNode(path.resolve(__dirname, '..', 'new', 'example_queries', 'redeemers'), name)
}
...
describe('redeemers', () => {
  ...
  it('can return an array of redeemers', async () => {
    ...
    util.saveResult(result.data, "new",   "redeemers", "redeemers.json");
  ...
  it('can return aggregated data', async () => {
    ...
    util.saveResult(result.data, "new",   "redeemers", "redeemersAggregate.json");
