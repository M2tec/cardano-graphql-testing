...
function loadQueryNode (name: string): Promise<DocumentNode> {
  return util.loadQueryNode(path.resolve(__dirname, '..', 'new', 'example_queries', 'stake_deregistrations'), name)
}
...
describe('stakeDeregistrations', () => {
  ...
  it('can return details for stake registrations', async () => {
    ...
    util.saveResult(result.data, "new",   "stake_deregistrations", "stakeDeregistrationsSample.json");
  ...
  it('can return aggregated data on all stake registrations', async () => {
    ...
    util.saveResult(result.data, "new",   "stake_deregistrations", "aggregateStakeDeregistrations.json");
