...
function loadQueryNode (name: string): Promise<DocumentNode> {
  return util.loadQueryNode(path.resolve(__dirname, '..', 'new', 'example_queries', 'delegations'), name)
}
...
describe('delegations', () => {
  ...
  it('can return details for stake delegation', async () => {
    ...
    util.saveResult(result.data, "new",   "delegations", "delegationSample.json");
  ...
  it('can return aggregated data on all delegations', async () => {
    ...
    util.saveResult(result.data, "new",   "delegations", "aggregateDelegation.json");

    // const { delegations_aggregate } = (result.data as any)
    // FIX FAIL expect(parseInt(delegations_aggregate.aggregate.count)).toBeGreaterThan(900)
  })
})
