...
function loadQueryNode (name: string): Promise<DocumentNode> {
  return util.loadQueryNode(path.resolve(__dirname, '..', 'new', 'example_queries', 'collateral_outputs'), name)
}
...
describe('collateralOutputs', () => {
  ...
  it('can return an array of collateral outputs', async () => {
    ...
    util.saveResult(result.data, "new",   "collateral_outputs", "collateralOutputs.json");
  ...
  it('can return aggregated data', async () => {
    ...
    util.saveResult(result.data, "new",   "collateral_outputs", "collateralOutputsAggregate.json");
