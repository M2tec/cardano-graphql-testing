...
function loadQueryNode (name: string): Promise<DocumentNode> {
  return util.loadQueryNode(path.resolve(__dirname, '..', 'new', 'example_queries', 'rewards'), name)
}
...
describe('rewards', () => {
  ...
  it('can return details for rewards scoped to an address', async () => {
    ...
    util.saveResult(result.data, "new",   "rewards", "rewardsForAddress.json");
  ...
  it('can return aggregated data on all delegations', async () => {
    ...
    util.saveResult(result.data, "new",   "rewards", "aggregateRewards.json");
