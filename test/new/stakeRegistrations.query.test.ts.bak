...
function loadQueryNode (name: string): Promise<DocumentNode> {
  return util.loadQueryNode(path.resolve(__dirname, '..', 'new', 'example_queries', 'stake_registrations'), name)
}
...
describe('stakeRegistrations', () => {
  ...
  it('can return details for stake registrations', async () => {
    ...
    util.saveResult(result.data, "new",   "stake_registrations", "stakeRegistrationsSample.json");
  ...
  it('can return aggregated data on all stake registrations', async () => {
    ...
    util.saveResult(result.data, "new",   "stake_registrations", "aggregateStakeRegistrations.json");
