...
function loadQueryNode (name: string): Promise<DocumentNode> {
  return util.loadQueryNode(path.resolve(__dirname, '..', 'orig', 'example_queries', 'blocks'), name)
}
...
describe('blocks', () => {
  ...
  it('caps the response to 100 blocks', async () => {
    ...
    util.saveResult(result.data, "orig", "blocks", "blockHashesNoArgs.json");
  ...
  it('allows custom pagination size with a limit and offset', async () => {
    ...
    util.saveResult(page1.data, "orig", "blocks", "first20Blocks.json");

    const page2 = await client.query({
      query: await loadQueryNode('second20Blocks')
    })

    util.saveResult(page2.data, "orig", "blocks", "second20Blocks.json");
  ...
  it('Can return blocks by number', async () => {
    ...
    util.saveResult(result.data, "orig", "blocks", "blockByNumbers.json");
  ...
  it('Can return aggregated data', async () => {
    ...
    util.saveResult(result.data, "orig", "blocks", "aggregateDataWithinBlock.json");
  ...
  it('Can return filtered aggregated data', async () => {
    ...
    util.saveResult(result.data, "orig", "blocks", "cant_return_filtered_aggregated_data.json");
  ...
  it('are linked to their predecessor, and the chain can be traversed', async () => {
    ...
    util.saveResult(result.data, "orig", "blocks", "selectGreatGrandparentBlock.json");

    expect((result.data as any).blocks[0].previousBlock.previousBlock.previousBlock.number).toBe(29019)
    expect((result.data as any)).toMatchSnapshot()
  })
  ...
  it('are linked to their successor, and the chain can be traversed', async () => {
    ...
    util.saveResult(result.data, "orig", "blocks", "selectGreatGrandchildBlock.json");

    expect((result.data as any).blocks[0].nextBlock.nextBlock.nextBlock.number).toBe(29025)
    expect((result.data as any)).toMatchSnapshot()
  })
})
