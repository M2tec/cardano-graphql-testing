...
function loadQueryNode (name: string): Promise<DocumentNode> {
  return util.loadQueryNode(path.resolve(__dirname, '..', 'orig', 'example_queries', 'collateral_inputs'), name)
}
...
describe('collateralInputs', () => {
  ...
  it('can return an array of collateral inputs', async () => {
    ...
    util.saveResult(result.data, "orig", "collateral_inputs", "collateralInputs.json");
  ...
  it('can return aggregated data', async () => {
    ...
    util.saveResult(result.data, "orig", "collateral_inputs", "collateralInputsAggregate.json");
